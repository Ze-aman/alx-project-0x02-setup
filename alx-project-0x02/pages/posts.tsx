...
import PostCard from '@/components/common/PostCard'; // <-- UPDATED TO USE ALIAS
import { type PostCardProps, type PostsPageProps } from '../interfaces'; // <-- UPDATED TO USE 'import type'

const PostsPage: React.FC<PostsPageProps> = ({ posts }) => {
  return (
    <div className="flex flex-col min-h-screen">
      <Head>
        <title>API Posts (SSG)</title>
      </Head>
      ...
      <main className="flex flex-col items-center w-full flex-1 px-4 sm:px-20 py-10">
        ...
        <p className="mt-2 text-xl text-gray-500 mb-10 text-center">
          Data fetched at build time using **`getStaticProps`** (Static Site Generation).
        </p>
...
export const getStaticProps: GetStaticProps<PostsPageProps> = async () => {
  try {
    ...
    return {
      ...
      // Optional: Regenerate the page every 1 hour (3600 seconds) in production
      revalidate: 3600, 
    };

  } catch (error) {
    console.error('Data fetching error:', error);
    // Return an empty array on error and set notFound to true if needed
    return {
      props: {
        posts: [],
      },
